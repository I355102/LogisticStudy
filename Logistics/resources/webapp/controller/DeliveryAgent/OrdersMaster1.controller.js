...
sap.ui.define([
		...
		"CaseStudy/Logistics/controller/DeliveryAgent/OrdersDetail.controller",'sap/m/MessageToast'
	], function (BaseController,JSONModel, History, Filter, Sorter, FilterOperator, GroupHeaderListItem, Device, formatter, Fragment, DetailController,MessageToast) {
		"use strict";
		...
		return BaseController.extend("CaseStudy.Logistics.controller.DeliveryAgent.OrdersMaster", {
      	...
      	oModel.read("/agents?$filter= F_Delivery_Id eq '" + localStorage.getItem("userId")+"'",{
      	...
      	return oDeferred.promise();
			...
			_onObjectMatched : function (oEvent) {
				//create ODATA service to retrieve Orders of DA logged in
			...
			var myPromise=this.getOdataPromise(ordersModel);
				...
				$.when(myPromise).done(
      			function(oData){
      				var ordersDataModel = new JSONModel({orders:oData});
      				this.getView().setModel(ordersDataModel,"orders");
      			}.bind(this));
			...
			onOpenViewSettings : function (oEvent){
				if (!this._oViewSettingsDialog) {
					this._oViewSettingsDialog = sap.ui.xmlfragment("CaseStudy.Logistics.view.ViewSettingsDialog", this);
			...
			_showDetail : function (oItem) {
				...
				this.getRouter().navTo("deliveryagent_orders_detail", {
					Order_Id : oItem.getBindingContext("orders").getProperty("Order_Id")
				}, true);
			},
			...
			if (!this._oPopover) {
				this._oPopover = sap.ui.xmlfragment(this.getView().getId(),"CaseStudy.Logistics.Fragment.DeliveryAgent.Popover", this);
		...
		handleSendButton :function(){
			//MessageToast.show("Request Sent");
               ...
               success: function(data) {
                  ...
                  //onRouteifSuccess();
                  formatter.show("Leave Created Successfully");            
               },
               error: function(data) {
